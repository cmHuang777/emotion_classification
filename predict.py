...
def predict(model, tokenizer, ds, max_new_tokens, temperature=0.6, repetition_penalty=1, combine_prompts=True):
    ...
    if combine_prompts:
        for out in pipe(KeyDataset(ds, "emotion_and_sentiment_prompt"), batch_size=BATCH_SIZE, return_full_text=False):
            ...
            counter += 1
        print(f"Total time elapsed generating emotion and sentiment (s): {(last_time-start_time).total_seconds()}")
        ...
        for out in pipe(KeyDataset(ds, "emotion_prompt"), batch_size=BATCH_SIZE, return_full_text=False):
            ...
            last_time = datetime.now()
            counter += 1
        print(f"Total time elapsed generating emotion (s): {(last_time-start_time).total_seconds()}")
        ...
        for out in pipe(KeyDataset(ds, "sentiment_prompt"), batch_size=BATCH_SIZE, return_full_text=False):
            ...
            raw = out[0]["generated_text"]
            llama3_sentiment = extract_label(raw, target_labels=["positive", "negative", "neutral"])
            llama3_labels[counter].update({"llama3_sentiment": llama3_sentiment})
            llama3_raws[counter].update({"llama3_sentiment_raw": raw})

            last_time = datetime.now()
            counter += 1
        print(f"Total time elapsed generating sentiment (s): {(last_time-start_time).total_seconds()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, help="Model serialization folder")
