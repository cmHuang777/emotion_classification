...
EMOTION_SENTIMENT_CONTEXT_WITH_EXAMPLES = (
    EMOTION_SENTIMENT_CONTEXT + EMOTION_SENTIMENT_EXAMPLES
)
...
def pred_arrays_to_csv(data_file, outfile1, outfile2, raws, labels) -> None:
    ...
    """
    with open(data_file, "r", newline="") as infile, open(
        outfile1, "w", newline=""
    ) as out_file1, open(outfile2, "w", newline="") as out_file2:
        csv_reader = csv.DictReader(infile)

        fieldnames1 = csv_reader.fieldnames + list(
            raws[0].keys()
        )  # columns depends on whether emotion and sentiment prompts are merged
...
def transform_text(example, include_roles=False, few_shots=True):
    if include_roles:
        ...
        if few_shots:
            example["emotion_prompt"] = put_in_role_msg(
                EMOTION_CONTEXT_WITH_EXAMPLES, user_msg
            )
            example["sentiment_prompt"] = put_in_role_msg(
                SENTIMENT_CONTEXT_WITH_EXAMPLES, user_msg
            )
            example["emotion_and_sentiment_prompt"] = put_in_role_msg(
                EMOTION_SENTIMENT_CONTEXT_WITH_EXAMPLES, user_msg
            )
        else:
            ...
            example["emotion_and_sentiment_prompt"] = put_in_role_msg(
                EMOTION_SENTIMENT_CONTEXT, user_msg
            )
    else:
        example["emotion_prompt"] = generate_emotion_prompt(
            example["text"], few_shots=few_shots
        )
        example["sentiment_prompt"] = generate_sentiment_prompt(
            example["text"], few_shots=few_shots
        )
        example["emotion_and_sentiment_prompt"] = generate_emotion_and_sentiment_prompt(
            example["text"], few_shots=few_shots
        )
    return example
