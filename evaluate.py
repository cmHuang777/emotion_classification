...
def evaluate(y_true, y_pred, labels):
    ...
    print(
        classification_report(
            y_true, y_pred, labels=labels, zero_division=0.0, digits=5
        )
    )
        ...
        for model in dir.iterdir():
            if model.is_dir():
                ...
                for item in model.iterdir():
                    ...
                    if (
                        pred_file.exists()
                        and not emotion_report_file.exists()
                        and not sentiment_report_file.exists()
                    ):
                        ...
                        emotion_report = evaluate(
                            golden["voted_emotion"],
                            predictions["llama3_emotion"],
                            emotion_labels,
                        )
                        sentiment_report = evaluate(
                            golden["voted_sentiment"],
                            predictions["llama3_sentiment"],
                            sentiment_labels,
                        )
                        ...
                        with open(item / "sentiment_report.json", "w") as f2:
                            json.dump(
                                sentiment_report, f2, indent=2, ensure_ascii=False
                            )
