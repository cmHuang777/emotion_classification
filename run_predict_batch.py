...
for item in Path("./checkpoint").iterdir():
    if item.is_dir():
            ...
            for rep_pen in [1.05, 1.1, 1.15, 1.2]:
                for max_new_tokens in [30, 50, 100]:
                    for temperature in [0.01, 0.1, 0.3, 0.5]:
                        out_dir = f"output/{dataset_name}/{model_id}/max_new_toks={max_new_tokens}-temp={temperature}-rep_pen={rep_pen}-combine_prompts={combine_prompt}-few_shots={few_shots}"
                        ...
                        cmd = f"python predict.py --model meta-llama/Meta-Llama-3-8B-Instruct --test_data {test_path} \
                            ...
                            --repetition_penalty {rep_pen} --combine_prompts={combine_prompt} \
                            --few_shots={few_shots} --output={str(out_dir)}| tee {out_dir}/predict.log.txt"
        ...
        # CUDA_VISIBLE_DEVICES=5 python local_llama_inference.py --model meta-llama/Meta-Llama-3-8B-Instruct --test_data data/drone/masked_all_tweets.csv